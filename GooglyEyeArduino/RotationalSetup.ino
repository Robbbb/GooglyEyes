...
void rotarySetup() {
  hallArraySetup();
  rotationalMotorController.attach(
      10);  // attaches the servo on pin 9 to the servo object
...
boolean spinTo(int target) {
  ...
  if (lastKnownSpinPosition == -1)
    spinClockwise();  // if it doesn't know where it is, spin it
  ...
  int fastestRotaryRoute =
      cleanRotaryGoal(spinThetaGoal, lastKnownSpinPosition);
...
// const int rotarySpinDesiredMagnitude = 1000;//the greatest allowed distance
// from stop
const int rotarySpinDesiredMagnitude =
    1250;  // the greatest allowed distance from stop
const int rotarySpinMaximumMagnitude =
    500;  // the greatest allowed distance from stop
...
void spinAt(int speed) {
  // Takes a speed from -8 to 8, and does a wacky linear mapping so it is
  // proportionally faster the further away form its goal it is.
  int addend = int(map(
      speed, -8, 8, -rotarySpinDesiredMagnitude, rotarySpinDesiredMagnitude));
  addend = constrain(
      addend, -rotarySpinMaximumMagnitude, rotarySpinMaximumMagnitude);
...
int HallArrayIdentifier() {
  ...
  for (int i = 0; i < positionQty; i++) {
    ...
    if (isLeftEye) {  // left eye was built 15 or so degrees off of the right
      // eye.
      spinPosition = (i + 1) % 16;
    } else {
      spinPosition = i;
...
boolean checkForFalseHits(int spinPositionCandidate) {
  if (lastKnownSpinPosition == -1)
    return true;  // Should only be the case at boot
  int jumpSize =
      abs(cleanRotaryGoal(spinPositionCandidate, lastKnownSpinPosition));
  if (jumpSize > 1) {
    return false;
  }
  // if suspected index found, make sure it is within reason
  // while eye is spinning, sometimes not all hall sensors in array have settled
  // on state at the same time, check that new vals make sense
  return true;
}

void hallArraySetup() {
  // The Hall effect sensors are active low, meaning the absensce of a magnet
  // casues a zero reading
  pinMode(2, INPUT);
  pinMode(4, INPUT);
  pinMode(5, INPUT);
  pinMode(6, INPUT);
...
void testCleanRotaryGoal() {
  ...
  if (cleanRotaryGoal(0, 1) == -1)
    Serial.println("(cleanRotaryGoal(0,1)       == -1) ");
  else
    Serial.println("failed!");
  if (cleanRotaryGoal(1, 0) == 1)
    Serial.println("(cleanRotaryGoal(1,0)       ==  1) ");
  else
    Serial.println("failed!");
  if (cleanRotaryGoal(2, 1) == 1)
    Serial.println("(cleanRotaryGoal(2,1)       == -1) ");
  else
    Serial.println("failed!");
  if (cleanRotaryGoal(1, 2) == -1)
    Serial.println("(cleanRotaryGoal(1,2)       ==  1) ");
  else
    Serial.println("failed!");
  if (cleanRotaryGoal(0, 15) == 1)
    Serial.println("(cleanRotaryGoal(0,15)      ==  1) ");
  else
    Serial.println("failed!");
  if (cleanRotaryGoal(15, 0) == -1)
    Serial.println("(cleanRotaryGoal(15,0)      == -1) ");
  else
    Serial.println("failed!");
  if (abs(cleanRotaryGoal(7, 15)) == 8)
    Serial.println("(abs(cleanRotaryGoal(7,15)) ==  8) ");
  else
    Serial.println("failed!");
  if (abs(cleanRotaryGoal(15, 7)) == 8)
    Serial.println("(abs(cleanRotaryGoal(15,7)) ==  8) ");
  else
    Serial.println("failed!");
  if (abs(cleanRotaryGoal(8, 0)) == 8)
    Serial.println("(abs(cleanRotaryGoal(8,0))  ==  8) ");
  else
    Serial.println("failed!");
  if (abs(cleanRotaryGoal(0, 8)) == 8)
    Serial.println("(abs(cleanRotaryGoal(0,8))  ==  8) ");
  else
    Serial.println("failed!");
  if (abs(cleanRotaryGoal(4, 12)) == 8)
    Serial.println("(abs(cleanRotaryGoal(4,12)) ==  8) ");
  else
    Serial.println("failed!");
  if (abs(cleanRotaryGoal(12, 4)) == 8)
    Serial.println("(abs(cleanRotaryGoal(12,4)) ==  8) ");
  else
    Serial.println("failed!");
