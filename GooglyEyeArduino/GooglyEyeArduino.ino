...
AccelStepper stepper(2,12,13);//initialize a two-wire stepper on pins 12 and 13
...
volatile int hiLimitValue; //state of linear limit switch
volatile int centerLimitValue; //state of linear limit switch
volatile int lowLimitValue; //state of linear limit switch

volatile long hiLimitSteps = 1600; //maximum linear steps from origin we can reasonably expect before tripping a limit switch
volatile long loLimitSteps = -1600;

volatile int overUnderOn = -1;//linear actuator relation to origin. -1 under 0 on 1 over.
// volatile int i = 0;
volatile long linearStepperGoal =0;

volatile int rotaryPositionGoal = 6;
volatile int rotaryPositionMillis;
volatile int lastKnownSpinPosition = -1;//was 6 a moment ago
volatile int lastKnownLateralPosition = 0;//always known!
