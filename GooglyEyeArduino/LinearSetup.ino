...
void linearSetup() {
  ...
  pinMode(
      pwmA,
      OUTPUT);  // init the 4 motor control pins for the shield to be outputs
  pinMode(pwmB, OUTPUT);
  pinMode(brakeA, OUTPUT);
  pinMode(brakeB, OUTPUT);

  pinMode(hiLimitPin,
          INPUT);  // init the three breakbeam limit switches as inputs
  ...
  digitalWrite(
      brakeA,
      LOW);  // We are using AccelStepper, so we just disable the brakes
  ...
  stepper.setMaxSpeed(
      stepperSpeed);  // this has no effect other than preventing overflow
...
bool updateLinearLimits() {
  ...
  if (centerLimitValue == crossedBeam)
    return true;
  else if (lowLimitValue == crossedBeam)
    return false;
  else if (hiLimitValue == crossedBeam)
    return false;
  else
    return true;
}
...
void enableStepper() {
  digitalWrite(
      pwmA,
      HIGH);  // We are using AccelStepper, so we just enable at full tilt(5v)
...
void disableStepper() {
  ...
  digitalWrite(
      pwmA,
      LOW);  // We are using AccelStepper, so we just enable at full tilt(5v)
